      ...
      if (connectionType === "WIFI" || connectionType === "BOTH") {
        try {
          const response = await fetch(
            `/api/devices/discover?type=WIFI&timeout=15000`
          );
          ...
          if (response.ok) {
            ...
            if (errorData.error) {
              setError(`WiFi: ${errorData.error}`);
            }
          }
        } catch (err) {
          console.error("Erreur découverte WiFi:", err);
          if (err instanceof Error) {
            setError(`WiFi: ${err.message}`);
          }
        }
      }
          ...
          {!discovering && devices.length === 0 && !error && (
            <div className="space-y-2 text-center">
              ...
              <p className="text-xs text-zinc-400">
                ⚠️ La détection réelle peut prendre jusqu'à 15 secondes. Assurez-vous que vos devices sont allumés et sur le même réseau.
              </p>
            </div>
          )}
